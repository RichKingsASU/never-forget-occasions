...
const TEMPLATES: Template[] = [
  {
    ...
    duration: "0:20",
    vibe: "Playful · upbeat",
    hue: "from-corten/40 to-primary/30",
  },
  {
    id: "t2",
    title: "Slow Sunday",
  ...
  },
  {
    ...
    duration: "0:25",
    vibe: "Cozy · nostalgic",
    hue: "from-corten/50 to-amber-500/30",
  },
  {
    id: "t4",
    title: "First Day",
  ...
  },
  {
    ...
    duration: "0:15",
    vibe: "Minimal · intimate",
    hue: "from-muted to-primary/20",
  },
  {
    id: "t6",
    title: "Big Reveal",
    ...
    vibe: "Bold · cinematic",
    premium: true,
    hue: "from-corten/50 to-primary/40",
  },
  {
    id: "t7",
    title: "Sunset Letter",
    category: "Anniversary",
    duration: "0:28",
    vibe: "Romantic · golden",
    hue: "from-amber-500/40 to-corten/30",
  },
  {
    id: "t8",
    ...
    category: "Birthday",
    duration: "0:22",
    vibe: "Retro · handcrafted",
    hue: "from-primary/30 to-accent/40",
  },
  {
    id: "t9",
    ...
    duration: "0:18",
    vibe: "Quiet · snowfall",
    hue: "from-accent/40 to-background",
  },
];
      ...
      TEMPLATES.filter(
        (t) =>
          ...
          (q === "" ||
            ...
            t.vibe.toLowerCase().includes(q.toLowerCase())),
      ),
    [q, cat],
  );
          ...
          <div className="mx-auto max-w-[1500px] space-y-5">
            <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Hand-crafted looks for every kind of occasion. Pick one and
                  let the AI do the words.
                </p>
              </div>
              <Button
                className="bg-corten text-white hover:bg-corten/90"
                onClick={() =>
                  toast.info("Custom template builder coming soon")
                }
              >
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                {CATEGORIES.map((c) => (
                  <button
                    key={c}
                    onClick={() => setCat(c)}
                    className={`rounded-full border px-3 py-1 text-xs transition ${cat === c ? "border-corten/60 bg-corten/15 text-corten" : "border-border/60 text-muted-foreground hover:text-foreground"}`}
                  >
                    {c}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((t) => (
                <Card
                  key={t.id}
                  className="glass-panel group overflow-hidden border-0 p-0 transition hover:-translate-y-0.5"
                >
                  <div
                    className={`relative aspect-video overflow-hidden bg-gradient-to-br ${t.hue}`}
                  >
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <button
                        ...
                        onClick={() =>
                          toast.success(`Loading preview · ${t.title}`)
                        }
                    ...
                    <div className="absolute left-3 top-3 flex gap-1.5">
                      ...
                      {t.premium && (
                        <Badge className="h-5 border-0 bg-corten/90 px-1.5 font-mono text-[10px] text-white">
                          PRO
                        </Badge>
                      )}
                    </div>
                    ...
                    >
                      <Heart
                        className={`h-4 w-4 ${favs.has(t.id) ? "fill-corten text-corten" : "text-muted-foreground"}`}
                      />
                    </button>
                  </div>
                  <div className="p-4">
                    ...
                    <Button
                      size="sm"
                      className="mt-3 w-full bg-corten text-white hover:bg-corten/90"
                      onClick={() =>
                        toast.success(`Using template · ${t.title}`)
                      }
                    >

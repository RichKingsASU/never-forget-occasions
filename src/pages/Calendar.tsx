...
import { ChevronLeft, ChevronRight, CalendarDays, Sparkles, Gift, Package, Loader2 } from "lucide-react";
...
export const Calendar = () => {
  ...
  const { data: rawOccasions = [], isLoading: occasionsLoading, error: occasionsError } = useOccasions();
  const { data: contacts = [], isLoading: contactsLoading } = useContacts();

  // Create lookup map of contactId -> name
  const contactsMap = useMemo(() => {
    return new Map(contacts.map((c) => [c.id, c.name]));
  }, [contacts]);

  // Map database occasions to UI occasions with contact names
  const mappedOccasions = useMemo(() => {
    return rawOccasions.map((o) => ({
      ...o,
      contactName: contactsMap.get(o.contact_id) || "Someone",
    }));
  }, [rawOccasions, contactsMap]);
  ...
  const { cells, monthOccasions } = useMemo(() => {
    ...
    const monthOccasions = mappedOccasions
      .filter((o) => {
  ...
  }, [cursor, mappedOccasions]);

  const occByDay = useMemo(() => {
    const map = new Map<number, typeof mappedOccasions>();
      ...
      <div className="lg:ml-64">
        <main className="px-4 py-6 md:px-8">
          <div className="mx-auto max-w-[1500px] space-y-5">
            <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {occasionsLoading ? "Loading occasions..." : `${monthOccasions.length} occasion${monthOccasions.length === 1 ? "" : "s"} this month`}
                </p>
              </div>
            ...
            {(occasionsLoading || contactsLoading) ? (
              <div className="flex h-96 items-center justify-center">
                <Loader2 className="h-8 w-8 animate-spin text-corten" />
              </div>
            ) : occasionsError ? (
              <Card className="glass-panel border-0 p-12 text-center text-destructive">
                          ...
                          <li key={o.id} className="flex items-center gap-3 py-3">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="truncate text-xs text-muted-foreground">
                                {o.type} · {formatDate(o.date)}
                                {d >= 0 && ` · in ${d}d`}
                              </p>
                              <div className="mt-1 flex items-center gap-1.5">
                                <Badge className={`h-4 border-0 px-1.5 font-mono text-[9px] uppercase tracking-wider ${statusTone[o.status] || statusTone["draft"]}`}>
                                  {o.status}
                                </Badge>
                              </div>
                            </div>
